...
public class FormationOffensive : MonoBehaviour
{
    ...
    private void AddBonus()
    {
        ...
        if (ApplyBonus.Count > 0)
        {
            foreach (GameObject id in ApplyBonus)
            {
                ...
                {
                    HasBonus.Add(U);
                    GetComponent<UnitScript>().DiceBonus++;
                    GetComponent<UnitScript>().PermaDiceBoost++;
                }
            }
        }
        WaitBonusEnd = false;
    ...
    {
        WaitBonusEnd = true;
        GetComponent<UnitScript>().DiceBonus -= HasBonus.Count;
        GetComponent<UnitScript>().PermaDiceBoost -= HasBonus.Count;
