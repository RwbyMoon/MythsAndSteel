...
public class Acceleration : MonoBehaviour
{
    ...
    void Update()
    {
        if(GetComponent<UnitScript>().NewTurnHasStart && !BoostLaunch)
        {
            ...
            {
                if((TilesManager.Instance.TileList[i].GetComponent<TileScript>().TerrainEffectList.Contains(MYthsAndSteel_Enum.TerrainType.Feu) || TilesManager.Instance.TileList[i].GetComponent<TileScript>().TerrainEffectList.Contains(MYthsAndSteel_Enum.TerrainType.Brasier)) && !BoostAct)
                {
                    BoostAct = true;
