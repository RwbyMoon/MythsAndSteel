...
public class Motivation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        foreach (int T in PlayerStatic.GetNeighbourDiag(tileId, TilesManager.Instance.TileList[tileId].GetComponent<TileScript>().Line, true))
        {
    ...
    private void AddBonus()
    {
        ...
        if (ApplyBonus.Count > 0)
        {
            foreach (GameObject id in ApplyBonus)
            {
                ...
                {
                    HasBonus.Add(U);
                    U.GetComponent<UnitScript>().PermaDiceBoost++;
                    U.GetComponent<UnitScript>().DiceBonus++;
                }
            }
        }
        WaitBonusEnd = false;
